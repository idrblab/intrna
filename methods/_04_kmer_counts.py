...
class BasicCounter_2:
    ...
    def get_counts(self,):
        ...
        return df
            ...
            #     return df
        # if len(self.encode) == 2:
        #     feature = []
        #     for i, seq in enumerate(seqs):
        #         encode = self.encode[0]
        #         seq01 = seq.replace('A', encode[0]).replace('C', encode[1]).replace('G', encode[2]).replace('T', encode[3])
        #         feature += self.occurrences(self.counts[i], seq01)
        ...
        #     print('len(self.seqs)')
        #     print(len(self.seqs))
        #     data_count = np.array(feature).reshape((len(self.seqs), (2 ** self.k) * len(encode)))
        ...
        #     df = DataFrame(data=data_count, index=seqname, columns=columnanmes,dtype='double')
        #     df01 = df.iloc[:, df.columns.str.contains('0')]
        # if len(self.encode) == 2 and self.encode[1].count("1", 0, 3) != 1:
        #     feature = []
        #     for i, seq in enumerate(seqs):
        #         encode = self.encode[1]
        #         seq01 = seq.replace('A', encode[0]).replace('C', encode[1]).replace('G', encode[2]).replace('T', encode[3])
        #         feature += self.occurrences(self.counts[i], seq01)
        ...
        #     df = DataFrame(data=data_count, index=seqname, columns=columnanmes,dtype='double')
        #     T1 = pd.concat([df01, df], axis=1, join='inner')    
        #     return T1
